...
function orgdetail(id){
    ...
    var url = "<c:url value='/author.do?dealMethod=originupdated&categoryId="+categoryId+"&authorId="+id+"'/>";
  //  alert(categoryId); 
    
    win = showWindow("generalSystemSet", "来源设置", url, 250, 90, 370, 250);
}
...
function button_add_onclick(ee) {	 
  ...
  //    win = showWindow("generalSystemSet", "来源设置", "author.do?dealMethod=origindetail&categoryId="+categoryId+"", 250, 90, 370, 241);	
      win = showWindow("generalSystemSet", "来源设置", "author.do?dealMethod=origindetail&categoryId="+categoryId+"", 250, 90, 370, 241);
...
function button_update_onclick(ee)
{
	 var categoryId=$("#categoryId").val();
	 var id= $("#id").val()
	//alert(id+"11111111111");
    var url = "<c:url value='/author.do?dealMethod=update&authorId=&categoryId="+categoryId+"'/>";
    win = showWindow("generalSystemSet", "修改作者信息", url, 250, 90,370, 241);
}
...
function button_delete_onclick(ee) {	
	...
	if(ids == "" || ids == null) {
		...
		if(confirm("你确定要删除吗？")) {
			...
			document.all("dealMethod").value = "deleteOrign";	
...
<body>
   ...
   <form id="generalSystemSetForm" action="<c:url value="/author.do"/>" method="post" name="generalSystemSetForm">
	<div class="currLocation">系统设置→来源设置
	 </div>
		...
		<complat:grid ids="xx" width="200,100,300,*" head="来源设置,是否默认,创建时间,网站名" coltext="[col:2,true:启用,false:禁用]" 
