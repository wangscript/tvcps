...
<script type="text/javascript">
	...
	$(document)	.ready(//页面加载是该调用的方法，
					function() {
						...
						var imgName=rightFrame.$("#"+rightFrame.getId(currRowNum)+"_2").html(); //图片名字
						var imgFile=rightFrame.$("#"+rightFrame.getId(currRowNum)+"_3").html(); //图片的后缀名
						
						//设置图片左上角的标题，为图片的名字
						top.document.getElementById("imgFileTitle").innerHTML =imgName+"."+imgFile;
	...
	function clickNext(){
		...
		var imgName=rightFrame.$("#"+rightFrame.getId(++imgIndex)+"_2").html();//获取图片的名字
		var imgFile=rightFrame.$("#"+rightFrame.getId(imgIndex)+"_3").html();//获取图片后缀名称
		top.document.getElementById("imgFileTitle").innerHTML =imgName+"."+imgFile; //在左上角显示图片标题
	...
	function clickBack(){
			...
			var imgName=rightFrame.$("#"+rightFrame.getId(currRowNum)+"_2").html();//获取图片的名字
			var imgFile=rightFrame.$("#"+rightFrame.getId(currRowNum)+"_3").html();//获取图片后缀名称
			top.document.getElementById("imgFileTitle").innerHTML =imgName+"."+imgFile;//在左上角显示图片标题
