...
<style type="text/css">
	...
	<script type="text/javascript">
		...
		function button_add_onclick(ee) {
			...
			var articleJs = $("#articleJs").val();
			var isScaleImage = $("#isScaleImage").val();
			var selectValue = $("#category : selected").val();
			win = showWindow("newAttachment","上传脚本","<c:url value='/module/document_manager/jsFile/js_detail.jsp?articleJs="+ articleJs +"&isScaleImage="+ isScaleImage +"&nodeId="+ category +"'/>", 293, 0, 480, 450);	
		}
		...
		function sure() {	
			...
			if(firstWin == null || firstWin == "undefined") {
				...
				if(!articleAtta.isEmpty()) {
					...
					var returnvalue = proccessScriptValue(document.documentForm, "sure", arrNames, arrUrls);
...
<body>
	<form id="documentForm" action="<c:url value="/js.do"/>" method="post" name="documentForm" >	
        <input type="hidden" name="dealMethod" id="dealMethod" value="uploadScriptList"/>
		...
		<input type="hidden" name="articleJs" id="articleJs" value="${documentForm.articleJs}"/>
		   ...
		   <div id="Layer2" style="position:absolute;  overflow: scroll; left:4px; top:10px; width:642px; height:520px; ">
				...
				<complat:grid ids="xx" width="*,*" head="脚本名称,脚本类型" page="${documentForm.pagination}" form="documentForm" action="js.do"/>
		    </div>
			<div id="Layer3" style="position:absolute; left:655px; top:11px; width:236px; height:390px; z-index:1">
				<fieldset style="height:370"><legend>&nbsp;<font color=blue style="color:blue;font:12px;font-weight:bold">脚本属性</font>&nbsp;</legend>
					...
					<td colspan="2"> 
					    <select style="width:160px;height:340px;" class="input_select_normal"  size="20" id="linkText" name="linkText">
						</select>
