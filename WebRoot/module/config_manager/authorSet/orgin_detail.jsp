...
<script type="text/javascript">
	$(document).ready(function() {      
		...
		if(document.getElementById("message").value == "3"){
			rightFrame.window.location.href="<c:url value='/author.do?dealMethod=origin&categoryId="+categoryid+"&" + getUrlSuffixRandom() + "'/>";			
...
<form action="author.do"  method="post" name="generalSystemSetForm" id="generalSystemSetForm" >
   	<c:if test="${generalSystemSetForm.generalSystemSet.setContent== null}">
  		 <input type="hidden" name="dealMethod" id="dealMethod"  value="origindetail"/>
  	</c:if>
    ...
    <c:if test="${generalSystemSetForm.infoMessage!=2}">
    	<c:if test="${generalSystemSetForm.generalSystemSet.setContent!= null}">
    		<input type="hidden" name="dealMethod" id="dealMethod"  value="originupdate"/>
     	</c:if>
   	</c:if>
	...
	<table style="width:770px;; font:12px;" >
		<tr>
			<td class="td_left" width="90px;"><i>*</i>来源设置:</td>
			<td>
				<input type="text" name="generalSystemSet.setContent" id="infoSource" class="input_text_normal" style="width:230px;"   value="${generalSystemSetForm.generalSystemSet.setContent}" valid="string" tip="来源设置不能为空" />
			</td>
		...
		<tr>		
			<td colspan="2">
				<span style="margin:0 0 0 25px;"><font color="red">*&nbsp;注:一次添加多个来源时不能启用默认设置</font></span>
			</td>
		</tr>
		</c:if>
        <tr height="50px">
