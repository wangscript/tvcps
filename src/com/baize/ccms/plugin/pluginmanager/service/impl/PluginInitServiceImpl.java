 ...
 */
package com.baize.ccms.plugin.pluginmanager.service.impl;
...
import com.baize.ccms.plugin.pluginmanager.service.PluginInitService;
...
public class PluginInitServiceImpl implements PluginInitService {

	private final Logger log = Logger.getLogger(PluginInitServiceImpl.class);
	/** 用户dao */
	...
	public List<Object> getTreeList(String treeid,String userId,String siteId, boolean isUpSiteAdmin) {
		...
		String xmlPath = GlobalConfig.appRealPath+ GlobalConfig.functionUnitXmlPath;
   ...
   /**
	...
	//	List list = ReadSystemXml.getTreeList(newTreeIds, GlobalConfig.functionUnitXmlPath);
		List<Object> list = new ArrayList<Object>();
		String xmlPath = GlobalConfig.appRealPath+ GlobalConfig.functionUnitXmlPath;
		SAXReader saxReader = new SAXReader();
		try {
					...
					if(setSiteId.equals(siteId)){
						for(int j = 1 ; j < siteIdAndMenuId.length ; j++){
							...
							if(strSiteIdAndMenuId.length > 1){
								if(strSiteIdAndMenuId[1].equals("m008")){
