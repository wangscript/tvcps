...
<script>
	var keylis = new Array("贸易", "制裁", "改造", "中国", "经济", "中国经济", "拯救", "美国",
			"美国经济", "整顿", "银行业", "银行", "速度", "温家宝", "美中", "逆差", "贸易逆差", "结构性",
			"转移性", "机电", "进出口", "商会", "彩电", "倾销", "税率", "交涉", "东北", "亚洲",
			"亚洲经济", "论坛", "增长", "失业率", "失业", "回落", "批评", "限制", "纺织品", "进口",
			"加拿大", "统计", "移民", "收入", "差异", "中央银行", "中央", "商业银行", "政策", "措施",
			"经济发展", "发展", "日本", "日本经济", "复苏", "决定", "锻铸", "铁管", "管件", "征收",
			"反倾销", "垄断", "倾销税", "联合国", "联合", "实施", "伊拉克", "石油", "食品", "计划",
			"发展中国家", "发达国家", "国家", "家电", "电信", "产业", "电子", "信息", "产销", "俄罗斯",
			"检察官", "检察", "首富", "犯罪", "调查", "结束", "欧盟", "争端", "投资", "受挫", "改革",
			"委员会", "委员", "实行", "临时", "价格", "干预", "施行", "美联储", "警告", "通货膨胀",
			"膨胀", "下滑", "衰退", "季度", "扬言", "演示简单", "后台");
	var keydrop = "";
	function delHtmlTag(str) {
		return str.replace(/<[^>]+>/g, "");//去掉所有的html标记 
	}
	function lastIndexDemo(a) {
		var text = rightFrame.getKeyText();
		if (text == null || text == "") {
			alert("请先填写文章!");
	...
	function getkeywords(keys, titles) {
		...
		for (i = 0; i < titl; i++) {
			titles[i] = rightFrame.getKeyText();
		}
		...
		for (i = 0; i < outkeyarray.length; i++) {
			for (j = outkeyarray.length; j > i; j--) {
				if (outkeyarray[i] == outkeyarray[j]) {
					outkeyarray = outkeyarray.slice(0, j).concat(
							outkeyarray.slice(j + 1, outkeyarray.length));
					outtfxarray = outtfxarray.slice(0, j).concat(
							outtfxarray.slice(j + 1, outtfxarray.length));
				}
			}
	...
	function keyWindowClose() {
		closeWindow(rightFrame.getWin());
	}
</script>
</head>
<body onload="lastIndexDemo()">
 
<table id="tt" style="width:300px;" cellpadding="0" cellspacing="0" border="0" style="line-height:22px;">
	<tr>
		<td align="left" ><b>此文章包含关键词</b></td>
	</tr>
	<tr>
		<td align="left"><span id="outkey">&nbsp;</span>&nbsp;</td>
	</tr>
	<tr>
		<td align="left"><b>以上关键词对应权重为</b></td>
	</tr>
	<tr>
		<td align="left" ><span id="outtfx">&nbsp;</span>&nbsp;</td>
	</tr>
	<tr>
		<td align="left"><b>此文章包含停用词</b></td>
	</tr>
	<tr>
		<td align="left"><span id="outstop">&nbsp;</span>&nbsp;</td>
	</tr>
	<tr>
		<td align="left"><b>关键词中符合并可作为标引词的有</b></td>
	</tr>
	<tr>
		<td align="left"><span id="outword">&nbsp;</span>&nbsp;</td>
	</tr>
	<tr>
		<td align="left"><b>您的标引深度为</b></td>
	</tr>
	<tr>
		<td align="left"><span id="outdeep">&nbsp;</span>&nbsp;</td>
	</tr>
	<tr>
		<td align="left"><b>最后确定的标引词为</b></td>
	</tr>
	<tr>
		<td align="left"><span id="outkeyword">&nbsp;</span>&nbsp;</td>
	</tr>
	<tr>
		<td align="center"><input type="button" name="exit" class="btn_normal" id="exit" value="关闭"
	onclick="keyWindowClose()"></td>
	</tr>
</table>
